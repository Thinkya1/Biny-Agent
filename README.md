...
Biny 是一个本地优先的 AI Agent，在 macOS Desktop、TUI 和 CLI 中连接你的模型服务，协助编码、研究和文件处理。会话、配置和长期上下文默认保存在本机。
...
全局 `$BINY_AGENT_DIR/identity/`（默认 `~/.biny/agent/identity/`）保存 `SOUL.md`、`IDENTITY.md`、`STYLE.md` 和 `USER.md`。Desktop 的 **设置 → 记忆 → Agent 灵魂** 支持预览、编辑、提案审核和 Alma 只读导入；Memory、TELOS 与身份资料彼此分开。
